...
class RunningHubClient(GrsaiClient):
    ...
    def _remember_id(self, data: object) -> None:
        if not _is_json_object(data) or not ({"taskId", "task_id"} & data.keys()):
            return
        ...
        task_id = (
            str(raw_task_id)
            if (isinstance(raw_task_id, str) or (type(raw_task_id) is int and raw_task_id >= 0))
            else ""
        )
        if (
            not task_id
    ...
    def _validate(self, status: int, data: object, allow_created: bool = False) -> str:
        ...
        if not 200 <= status < 300:
            detail = data.get("errorMessage") if _is_json_object(data) else "Non-JSON response."
            raise self._error(
                f"HTTP {status}: {clean_message(detail or 'Request rejected.', self._secrets)}"
            )
    ...
    async def upload_image(
        self, session: aiohttp.ClientSession, content: bytes, index: int = 1
    ) -> str:
        t = self.config.transport
        for attempt in range(t.download_retry_limit + 1):
            form = aiohttp.FormData()
            form.add_field(
                "file", content, filename=f"reference-{index:02d}.png", content_type="image/png"
            )
            delay = None
            try:
                ...
                if status not in RETRYABLE_HTTP_STATUSES:
                    message = (
                        ...
                        else "Invalid upload response."
                    )
                    raise self._error(
                        f"Reference image upload failed: {clean_message(message, self._secrets)}",
                        index,
                    )
            except (TimeoutError, aiohttp.ClientError):
                if attempt == t.download_retry_limit:
                    raise self._error(
                        "Reference image upload failed after bounded retries.", index
                    ) from None
            if attempt == t.download_retry_limit:
    ...
    async def _upload_images(self, contents: list[bytes]) -> list[str]:
        async with aiohttp.ClientSession() as session:
            return [
                await self.upload_image(session, content, index)
    ...
    async def _generate(self, request: Mapping[str, object]) -> list[torch.Tensor]:
        ...
        async with self._sessions() as (api, cdn):
            ...
            for index, result in enumerate(results, 1):
                url = result.get("url") if _is_json_object(result) else None
                try:
