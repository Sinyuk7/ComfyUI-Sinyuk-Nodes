...
from ...features.image_api.batch_runner import BatchRunner
from ...features.image_api.config import get_config
from ...features.image_api.diagnostics import new_run_id
from ...features.image_api.references import load_folder
from ...features.image_api.workflow import provider_profile, scalar
...
class ImageAPILoadImagesFromFolder(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            node_id="Sinyuk.ImageAPI.LoadFolder",
            display_name="Image API Load Folder",
            category="Image API",
            is_input_list=True,
            description=(
                "Load an ordered, non-recursive image set from a folder on the ComfyUI server."
            ),
            ...
            inputs=[
                io.String.Input(
                    ...
                    display_name="Folder",
                    tooltip=(
                        "Folder on the ComfyUI server. Loads supported images non-recursively "
                        "in natural filename order; re-scans on every run and does not resize."
                    ),
                )
            ],
            outputs=[
                ReferenceType.Output(
                    ...
                    tooltip=(
                        "Ordered images plus source filenames for a Batch Image Generate "
                        "Reference input."
                    ),
                ),
                io.Image.Output(
                    ...
                    is_output_list=True,
                    tooltip=(
                        "The same ordered images as a standard IMAGE list for Preview, Save, "
                        "or processing nodes."
                    ),
                ),
            ],
...
class BatchImageGenerate(io.ComfyNode):
    ...
    def define_schema(cls):
        config = get_config()
        shared = [
            APIConfigType.Input("api_config", display_name="Config"),
            ...
            io.String.Input(
                "prompt", display_name="Prompt", default="", multiline=True, dynamic_prompts=False
            ),
        ]
        for input_, label in zip(shared, ("Config", "Model", "Prompt"), strict=True):
        ...
        template = io.Autogrow.TemplateNames(
            io.MultiType.Input(
                ...
                tooltip=(
                    "One ordered reference source. Connect a folder References output or a "
                    "standard IMAGE input."
                ),
            ),
            names=[f"reference_{i}" for i in range(1, config.batch_reference_limit + 1)],
            min=1,
        )
        return io.Schema(
            node_id="Sinyuk.ImageAPI.BatchGenerate",
            display_name="Image API Batch Generate",
            category="Image API",
            ...
            inputs=[
                io.Autogrow.Input(
                    ...
                    template=template,
                    tooltip=(
                        "Connect 1-10 ordered reference sources. Each task receives one image "
                        "from every connected source."
                    ),
                ),
                io.String.Input(
                    "prompts",
                    ...
                    force_input=True,
                    tooltip=(
                        "Optional STRING list. Nonempty values override Prompt and create "
                        "every base x prompt combination, not index pairing."
                    ),
                ),
                *shared,
                io.Int.Input(
                    ...
                    max=10,
                    tooltip=(
                        "Maximum paid generation tasks running at once (2-10). Higher values "
                ...
                io.String.Input(
                    ...
                    default="ImageAPI",
                    tooltip=(
                        "Filename prefix inside a new batch folder under ComfyUI output. A "
                        "manifest is saved beside the images."
                    ),
                ),
            ],
            outputs=[
                io.Image.Output(
                    ...
                    is_output_list=True,
                    tooltip=(
                        "Successful generated images flattened in task and result order; failed "
                        "tasks are omitted."
                    ),
                ),
                io.String.Output(
                    ...
                    tooltip=(
                        "Absolute path to the JSON manifest containing task mapping, output "
                        "files, remote IDs, and errors."
                    ),
                ),
            ],
    ...
    @classmethod
    async def execute(
        cls, references, api_config, model, prompt, max_concurrency, output_prefix, prompts=None
    ):
        import folder_paths
        from comfy import model_management
        ...
        concurrency, prefix = (
            scalar(max_concurrency, "max_concurrency"),
            scalar(output_prefix, "output_prefix"),
        )
